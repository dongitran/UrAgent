...
// ============================================================================
// ISandbox-compatible handlers (Provider-agnostic)
// These functions work with both Daytona and E2B providers
// ============================================================================

export async function handleViewCommandWithInstance(
  ...
  try {
    ...
    // Read file contents using ISandbox-compatible function
    const { success, output } = await readFileWithInstance({
      sandboxInstance,
      filePath: path,
      workDir,
      config,
...
export async function handleStrReplaceCommandWithInstance(
  sandboxInstance: ISandbox,
  config: GraphConfig,
  inputs: StrReplaceCommandInputs,
): Promise<string> {
  const { path, workDir, oldStr, newStr } = inputs;
  const { success: readSuccess, output: fileContent } = await readFileWithInstance({
    sandboxInstance,
    filePath: path,
    workDir,
    config,
  ...
  const { success: writeSuccess, output: writeOutput } = await writeFileWithInstance({
    sandboxInstance,
    filePath: path,
    content: newContent,
    workDir,
...
export async function handleCreateCommandWithInstance(
  sandboxInstance: ISandbox,
  config: GraphConfig,
  inputs: CreateCommandInputs,
): Promise<string> {
  const { path, workDir, fileText } = inputs;
  // Check if file already exists
  const { success: readSuccess } = await readFileWithInstance({
    sandboxInstance,
    filePath: path,
    workDir,
    config,
...
export async function handleInsertCommandWithInstance(
  sandboxInstance: ISandbox,
  config: GraphConfig,
  inputs: InsertCommandInputs,
): Promise<string> {
