 ...
 */
function hashToolCallArgs(args: Record<string, unknown> | null | undefined): string {
 ...
 */
function isReadOnlyToolCall(name: string, args: Record<string, unknown>): boolean {
 ...
 */
function extractTargetFile(name: string, args: Record<string, unknown>): string | null {
  ...
  if (name === "shell" && typeof args.command === "string") {
    ...
    const catMatch = command.match(/^(?:cat|head|tail|less|more)\s+(?:-[^\s]+\s+)*([^\s|>]+)/);
    ...
    if (!targetFile) {
      const grepMatch = command.match(/grep\s+(?:-[^\s]+\s+)*(?:"[^"]*"|'[^']*'|\S+)\s+([^\s|>]+)/);
    ...
    if (!targetFile) {
      const simpleMatch = command.match(/\s([^\s|>]+\.[a-zA-Z0-9]+)(?:\s*$|\s*\|)/);
...
function extractRecentToolCalls(messages: BaseMessage[]): ToolCallSignature[] {
  ...
  const recentMessages = messages.slice(-LOOP_DETECTION_CONFIG.MESSAGES_TO_ANALYZE);
 ...
 */
function extractToolCallsByMessage(messages: BaseMessage[]): ToolCallSignature[][] {
 ...
 */
function extractToolCallsWithResults(messages: BaseMessage[]): ToolCallWithResult[] {
  const toolCallsWithResults: ToolCallWithResult[] = [];
  const recentMessages = messages.slice(-LOOP_DETECTION_CONFIG.MESSAGES_TO_ANALYZE);
  
  // Build a map of tool_call_id to status, content, and output hash
  const statusMap = new Map<string, { status: "success" | "error" | "unknown"; content?: string; outputHash?: string; toolName?: string }>();
  ...
  for (const message of recentMessages) {
    if (isToolMessage(message) && message.tool_call_id) {
      ...
      } else if (message.status === "error" || hasErrorInContent || hasStrReplaceError) {
        effectiveStatus = "error";
      }
      
      // Create a simple hash of output for comparison
      const outputHash = content ? simpleHash(content) : undefined;
      statusMap.set(message.tool_call_id, { status: effectiveStatus, content, outputHash, toolName });
    }
  }
...
function detectChanting(messages: BaseMessage[]): {
    ...
    if (isAIMessage(message)) {
      const content = typeof message.content === "string" 
        ...
        : Array.isArray(message.content) 
          ? message.content.map(c => typeof c === "string" ? c : (c as any).text || "").join(" ")
          : "";
  ...
  for (let i = aiContents.length - 2; i >= 0; i--) {
    ...
    if (aiContents[i] === lastContent || 
        (aiContents[i].length > 50 && lastContent.length > 50 && 
         calculateSimilarity(aiContents[i], lastContent) > LOOP_DETECTION_CONFIG.CHANTING_SIMILARITY_THRESHOLD)) {
      count++;
    } else {
      break;
    }
  ...
  return {
    ...
    repeatedContent: count >= LOOP_DETECTION_CONFIG.CHANTING_MIN_COUNT ? lastContent.substring(0, 100) : null,
    count,
  };
}
...
function detectAlternatingPattern(toolCalls: ToolCallSignature[]): {
  ...
  if (toolCalls.length >= 6) {
    ...
    if (sig0 === sig2 && sig2 === sig4 && sig1 === sig3 && sig3 === sig5 && sig0 !== sig1) {
      return { isAlternating: true, patternLength: 6, cycleType: "2-element" };
    }
    
    // Check 3-element cycle: A→B→C→A→B→C
    if (sig0 === sig3 && sig1 === sig4 && sig2 === sig5 && sig0 !== sig1 && sig1 !== sig2 && sig0 !== sig2) {
    ...
    const isAllEdits = last6.every(tc => 
    ...
    if (isAllEdits && files[0] && files[1]) {
      ...
      const file1 = files[1];
      if (file0 !== file1 && 
          files[2] === file0 && files[3] === file1 && 
          files[4] === file0 && files[5] === file1) {
        // Oscillating between 2 files - this is suspicious
        return { isAlternating: true, patternLength: 6, cycleType: "oscillating" };
      }
    }
  }
  ...
  if (last4.length >= 4) {
    ...
    const isAllEdits = last4.every(tc => 
      tc.name === "str_replace_based_edit_tool" || tc.name === "apply_patch"
    );
    
    if (isAllEdits && files[0] && files[1] && files[0] !== files[1]) {
      if (files[2] === files[0] && files[3] === files[1]) {
        return { isAlternating: true, patternLength: 4, cycleType: "oscillating" };
      }
    }
  }
...
function detectReadOnlyLoop(toolCalls: ToolCallSignature[]): boolean {
  ...
  if (uniqueFiles.size >= LOOP_DETECTION_CONFIG.MIN_UNIQUE_FILES_FOR_EXPLORATION) {
    return false;
  }
  ...
  for (const [file, count] of fileAccessCount) {
    if (count >= 4 && uniqueFiles.size <= 2) {
      ...
      logger.info("Detected semantic read loop on file", { file, accessCount: count });
      return true;
    }
  }
...
function detectSimilarToolCalls(toolCalls: ToolCallSignature[]): {
  ...
  if (toolCalls.length < LOOP_DETECTION_CONFIG.SIMILAR_TOOL_THRESHOLD) {
    return { isSimilar: false, count: 0, targetFile: null, isEditLoop: false, editCount: 0 };
  }
  ...
  for (const tc of recentCalls) {
    ...
    if (targetFile) {
      ...
      } else if (isWriteToolCall(tc.name, tc.fullArgs) || tc.name === "str_replace_based_edit_tool") {
        fileWriteCount.set(targetFile, (fileWriteCount.get(targetFile) || 0) + 1);
      }
    }
  }
 ...
 */
function detectReadEditFailLoop(toolCalls: ToolCallSignature[], messages: BaseMessage[]): {
...
function detectErrorRetryLoop(
  ...
  const toolCallsWithResults = extractToolCallsWithResults(messages);
  if (toolCallsWithResults.length < LOOP_DETECTION_CONFIG.MIN_CALLS_FOR_ERROR_RATE) {
    return false;
  }
  
  // Check last N tool calls for error rate
  const recentCalls = toolCallsWithResults.slice(-LOOP_DETECTION_CONFIG.MIN_CALLS_FOR_ERROR_RATE);
  ...
  return errorCount >= Math.ceil(recentCalls.length * LOOP_DETECTION_CONFIG.ERROR_RATE_THRESHOLD);
}
...
function determineRecommendation(
  ...
  if (hasVaryingOutputs && loopType !== "chanting" && loopType !== "edit_loop") {
  ...
  if (loopType === "similar_calls") {
    if (similarCallsCount >= LOOP_DETECTION_CONFIG.FORCE_COMPLETION_THRESHOLD + 1) {
      return "force_complete";
    }
    return "warn";
  }
  ...
  if (consecutiveCount >= LOOP_DETECTION_CONFIG.FORCE_COMPLETION_THRESHOLD + 2) {
    return "force_complete";
  }
  if (consecutiveCount >= LOOP_DETECTION_CONFIG.LOOP_THRESHOLD) {
    return "warn";
...
export function detectLoop(messages: BaseMessage[]): LoopDetectionResult {
  ...
  if (isLegitimateBuildFixRetry(messages)) {
    logger.info("Detected legitimate build-fix-retry pattern, skipping loop detection");
    return {
      isLooping: false,
      loopCount: 0,
      repeatedToolCall: null,
  ...
  const { count: consecutiveCount, signature, hasVaryingOutputs } = countConsecutiveIdenticalCalls(toolCalls, messages);
  ...
  if (hasVaryingOutputs && consecutiveCount >= LOOP_DETECTION_CONFIG.LOOP_THRESHOLD) {
  ...
  if ((loopType === "similar_calls" || loopType === "edit_loop") && targetFile) {
    reportSignature = {
  ...
  if (isLooping) {
    logger.warn("Loop detected in agent behavior", {
      ...
      forceCompletionThreshold: LOOP_DETECTION_CONFIG.FORCE_COMPLETION_THRESHOLD,
    });
  } else {
...
function countPreviousWarnings(messages: BaseMessage[]): number {
  ...
  for (const message of recentMessages) {
    ...
    if (content.includes("CRITICAL_LOOP_WARNING") || content.includes("LOOP DETECTED")) {
      count++;
    }
  }
...
export function shouldSkipDueToLoop(
  ...
  if (lastCall.name === proposedToolName && lastCall.argsHash === proposedHash) {
...
export function isLegitimateBuildFixRetry(messages: BaseMessage[]): boolean {
  ...
  for (const tc of recentCalls) {
    const isBuildCommand = tc.name === "shell" && typeof tc.fullArgs.command === "string" && 
      /^(npm|yarn|pnpm)\s+(run\s+)?(build|test|lint|check|compile|tsc)/.test(tc.fullArgs.command.trim());
    
    const isEditCommand = tc.name === "str_replace_based_edit_tool" || tc.name === "apply_patch";
